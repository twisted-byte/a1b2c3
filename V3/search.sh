...
download_game() {
    local game_info="$1"
    decoded_name=$(echo "$game_info" | cut -d '|' -f 1)
    ...
    # Log the game info for download
...
search_games() {
    ...
    for file in $(find "$DEST_DIR" -type f -name "*.txt"); do
        ...
        while IFS="|" read -r decoded_name encoded_url game_download_dir; do
            ...
            if [[ "$decoded_name_lower" =~ $search_term ]]; then
                ...
                # Store the full line along with the folder name
                results+=("($folder_name) $game_name_cleaned|$file|$decoded_name|$encoded_url|$game_download_dir" "" off)
            fi
        done < "$file"
    done
    ...
    if [[ ${#results[@]} -gt 0 ]]; then
        ...
        for game in $selected_games; do
            # Extract the full line containing the game name, URL, and destination
            game_info=$(echo "$game" | cut -d '|' -f 3-)
